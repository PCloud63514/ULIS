...
public class NoticeServiceImpl implements NoticeService {
	...
	public Boolean isNoticeBoardAddNull(NoticeBoard noticeBoard) {
		...
		if (noticeBoard.getTitle() == null || noticeBoard.getTitle().trim().equals("")
				|| (noticeBoard.getGradeOne() == null && noticeBoard.getGradeTwo() == null
						&& noticeBoard.getGradeThree() == null && noticeBoard.getGradeFour() == null)
				|| noticeBoard.getEndDate() == null || noticeBoard.getEndDate().equals("")
				|| noticeBoard.getContent() == null || noticeBoard.getContent().trim().equals("")) {
			return false;
		}
		return true;
	}
	...
	public int addNotice(NoticeBoard noticeBoard) {
		...
		fcmAlarm(noticeBoard,String.valueOf(noticeBoard.getNoticeBoardNo()));
		return noticeBoard.getNoticeBoardNo();
	}

	private void fcmAlarm(NoticeBoard noticeBoard, String noticBoardNo) {
		// TODO 대상 조건 동일 학과 지정 학년들
		List<Student> studentList = new ArrayList<Student>();
		Student student = new Student();
		student.setSubjectNo(noticeBoard.getSubjectNo());

		if ("Y".equals(noticeBoard.getGradeOne())) {
			student.setGrade(1);
			studentList.addAll(studentMapper.fcmList(student));
		}
		if ("Y".equals(noticeBoard.getGradeTwo())) {
			student.setGrade(2);
			studentList.addAll(studentMapper.fcmList(student));
		...
		sendData.put("type", "notice");
		sendData.put("noticeBoardNo",noticBoardNo);
		
		for (Student studentFcm : studentList) {
			String fcmToken = studentFcm.getFcmToken() != null ? studentFcm.getFcmToken() : "";
			if (fcmToken != null && "".equals(fcmToken) == false) {
		...
		if (fcmList.size() > 0) {
			try {
				fcmService.push(fcmList, "공지사항 알림", noticeBoard.getTitle(), sendData);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	public NoticeBoardFile imageView(String physicsfileName, HttpServletResponse httpServletResponse) {
		...
		String filePath = this.path + File.separator + noticeBoardFile.getFilePath() + File.separator
				+ noticeBoardFile.getPhysicsfileName();
		int extensionLoc = noticeBoardFile.getLogicfileName().lastIndexOf(".");
		httpServletResponse.setHeader("Content-Disposition",
				"inline;filename=" + noticeBoardFile.getLogicfileName() + ";");
		httpServletResponse.setContentType("image/" + noticeBoardFile.getLogicfileName().substring(extensionLoc + 1,
				noticeBoardFile.getLogicfileName().length()));
